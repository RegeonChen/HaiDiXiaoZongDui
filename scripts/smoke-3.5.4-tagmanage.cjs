 ...
 *  2) 工具栏保留标签 + 笔记，独立“标签建议”入口已移除
 *  3) 点标签 → StickyBottomPanel 展开，显示标签管理与自动 AI 建议
 *  4) 点 ▾ 收起 → 面板折叠为只显示 tab bar
 *  5) 重新点标签 → 面板重新展开（高度可拉伸）
 *  6) 拖拽手柄 → 面板高度变化
 *  7) 标签栏自动调 mock AI，出现 1+ 条建议
 *  8) 点“应用”按钮 → tag 直接出现在同一栏的“已应用”列表
 *  9) 切到笔记 → textarea 与中性色按钮正常显示
 * 10) 经典、纸质主题下普通按钮均不使用白色背景
 *
