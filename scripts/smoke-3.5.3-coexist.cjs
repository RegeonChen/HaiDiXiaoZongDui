/**
 * Task 3.5.3 activePanel Set 化 smoke — 摘要 toggle + 摘要/翻译并存
 *
 * 验证（Phase 3.5.x 修复）：
 *   1) 点 ✨ 摘要 → 摘要按钮变为 🙈 隐藏摘要，悬浮窗渲染
 *   2) 再点 🙈 隐藏摘要 → 按钮回到 ✨ 摘要，悬浮窗消失
 *   3) 第三次点 ✨ 摘要 → 悬浮窗重新出现，**不重新调 AI**（复用缓存）
 *   4) 摘要打开时点 🌐 翻译 → 摘要悬浮窗 + 翻译视图同时存在
 *      （之前 activePanel 单值互斥，body 区域 if-else 切换会丢摘要）
 *   5) 关掉翻译 → 摘要悬浮窗仍保留
 *
