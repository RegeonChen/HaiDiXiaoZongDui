...
const DEFAULT_BRIEF_PROMPT = [
  ...
  '',
  'Language: {{language}}',
  'Return valid Markdown only. Do not wrap the response in a code fence.',
  '',
  'Article:',
  '{{content}}'
].join('\n');
...
const DEFAULT_STANDARD_PROMPT = [
  'You are a professional editor. Write a structured Markdown summary of the following article.',
  '',
  'Guidelines:',
  '- Capture the main argument, key evidence, and conclusion.',
  '- Keep technical accuracy; do not invent facts.',
  '- Use exactly these sections: `## Overview`, `## Key Points`, and `## Conclusion`.',
  '- Put the key points in a Markdown bullet list and use **bold** only for meaningful emphasis.',
  '- Return valid Markdown only. Do not wrap the response in a code fence.',
  '- Language: {{language}}',
  '',
  'Article:',
...
const DEFAULT_DETAILED_PROMPT = [
  ...
  '## Key Points (Markdown bullet list)',
  '## Evidence / Data (if any)',
