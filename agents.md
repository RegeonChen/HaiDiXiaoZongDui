...
- **v1.0.0 release 已发布**（`efb7626` + tag `v1.0.0`）：普通合并保留远端笔记底栏功能与本地 AI/错误处理、工作区布局改动，删除误提交的 `commit-msg.txt`，并将文摘功能移除纳入正式版。发布前通过双端 typecheck、203 项单测、生产构建、Phase 3 集成/摘要/翻译/文章 AI 对话 4 组关键 Electron smoke 和生产依赖审计。GitHub Actions run `30754761750` 成功生成并发布 `Juhe-Shiyi-1.0.0-arm64.dmg`（132,602,532 bytes，SHA-256 `c2e6f950e5d41e9fcf4b726d3d50a8584217823b884a56a102e49c22d6ed795c`）与 `Juhe-Shiyi-Setup-1.0.0-x64.exe`（114,644,573 bytes，SHA-256 `50de0afa35cd0042dc9d27610e01f01dd0d1c847df64f22f70fa2ad6b8d17d59`）。
...
- **文摘功能移除（`4ebabbf`，已纳入 v1.0.0）**：删除文摘页面、仓储、IPC、DataSource 和相关样式，新手教程从 12 步调整为 11 步。

- **AI 延迟与全链路中文错误提示加固（2026-08-02，已纳入 v1.0.0）**：逐段翻译从完全串行改为受控双并发，并按段落长度动态限制输出；摘要、标签和文章问答去除图片 URL，对超长正文保留开头/中段/结尾代表性内容，任务超时收紧为 45–60 秒。Main 进程统一分类 AI 鉴权、限流、模型、网络、代理、证书、空响应和格式错误；Renderer 将同步、Feed、OPML、磁盘、数据库及 AI 错误转换为带处理建议的中文文案，批量操作展示逐项失败原因。双端 typecheck、203 项单测、生产构建、摘要/翻译/标签/AI 对话、Feed actions、OPML 导出和 Phase 3 集成 Electron smoke 均通过；真实外部 Provider 摘要/翻译/标签仍受“无 API Key”环境限制而跳过。

- **功能工作区全屏布局统一（2026-08-02，已纳入 v1.0.0）**：标签、笔记、专题、OPML 导出、通用设置、AI 设置和专题详情统一使用 1040px 居中内容轴；展开目录或窄窗口时按可用宽度自然收缩，收起目录或全屏时不再出现部分页面铺满、部分页面居中的差异。专题详情同步统一为页头、标签栏与内容卡片结构。独立的 `workspace-pages.css` 在页面组件样式之后加载，避免历史页面宽度再次覆盖统一规则；`smoke:phase4.2` 在两级目录均收起时测得标签、笔记、专题宽度均为 1040px、左右留白均为 97px，OPML、专题和完整工作台 smoke 均通过。
...
- **订阅同步系统代理兼容修复（2026-08-02，已纳入 v0.3.5）**：Feed 同步和懒加载文章正文从 Node 全局 `fetch` 切换到 Electron `net.fetch`，使用 Chromium 网络栈并继承系统代理、PAC、HTTPS 隧道和代理认证能力；HTTP 客户端改为可注入网络实现，继续执行 http(s) 白名单、15/20 秒超时、一次重试和 5/10 MB 大小限制。DNS、代理、证书、拒绝连接、连接重置和超时会转换为稳定错误码与可读提示，本地日志只增加不含 URL 的失败码。真实 Feed smoke 同步修正为断言内层 `SyncResult.success`，并通过正文 IPC 验证实际阅读内容。164 项单测、双端 typecheck、生产构建、离线 Phase 2、UI/IPC 和 Feed actions smoke 均通过；用户报告失败的 `soulhacker.me`、`ericmigi.com`、`baoyu.io` 和 `seangoedecke.com` 四个真实源均在 Electron 完整链路中同步、读取正文和持久化成功。

- **AI 标签建议与摘要宽度兼容修复（2026-08-02，已纳入 v0.3.5）**：Tag Agent 关闭思考模式并请求结构化 JSON，兼容正式答案位于 `reasoning_content`、代码围栏、分析文字、尾逗号和旧版顶层数组的 OpenAI-compatible 模型响应；只有结构化任务会读取并校验该字段，普通摘要与对话仍不会暴露推理内容。无结构响应追加一次受控 JSON 修复，界面将内部错误码转换为可操作提示。摘要底部栏移除固定 `76ch` 上限，在收起一、二级目录后随灵活窗口扩展且保持在面板边界内；单元测试、真实 IPC smoke 和目录三态测量 smoke 覆盖这些回归。

- **AI 网络、专题推荐延迟与错误诊断加固（2026-08-02，已纳入 v0.3.5）**：所有 OpenAI-compatible 请求改由 Main 进程注入 Electron `net.fetch`，继承 Chromium 系统代理能力；同一 Provider/模型拒绝 `response_format` 后会在本次应用进程内记住兼容性，后续结构化任务直接降级，降级重试与首次请求共用同一超时预算。专题推荐正文输入从 12,000 字收紧到 6,000 字，首次请求最多 45 秒、仅在完全无法本地解析 JSON 时追加一次最多 15 秒的受控修复，输出预算分别为 1,000/700 tokens。Main 进程按超时、鉴权、限流、网络、Provider 故障、空响应、格式异常和无可用候选返回稳定错误码；本地脱敏日志只记录模型名、耗时、请求次数、修复/格式降级状态和错误类别，不记录文章、URL 或凭证。单元测试与 `smoke:task33` 覆盖真实 IPC 降级、缓存复用和日志脱敏。
...
- **IDE 工作台四段式布局（`8ca0998`，已发布）**：页面固定为“竖向功能栏 / 一级订阅源目录 / 二级文章目录 / 灵活窗口”。打开文章、标签、笔记、文摘、专题或设置时，前两级目录保持挂载且不被页面替换；仅最右灵活窗口切换内容。灵活窗口顶部保留可切换、可关闭的 IDE 标签条，但不创建永久“阅读器”标签。一级/二级目录分别设 218px/260px 最小宽度，工具按钮统一禁止文字换行。添加订阅源、导入 OPML、导出 OPML和新建订阅源组统一收进一级目录右上角“+”菜单。顶栏小三角、左上角重复阅读入口和右上角全局同步按钮均已移除；在阅读界面重复点击竖向首个“阅读”功能键，目录按“全开 → 收起一级 → 再收起二级 → 全开”循环，从其他页面点击则只返回阅读。“所有订阅源”与具体订阅源统一在二级目录标题下显示“同步 / 全部已读”，前者作用于全部源、后者只作用于当前源；未读、星标和标签筛选不显示范围含糊的批量操作。`Layout.test.ts` 验证固定结构、最小宽度、重复入口移除和三态目录，`FeedList.test.ts` 锁定“+”菜单操作。
- **灵活窗口采用 VS Code 预览标签语义**：普通单击文章、标签、笔记、文摘、专题或设置时，只复用一个斜体预览标签，后续普通打开原位替换；双击文章或双击标签页后转为固定标签，固定项不再被预览替换。关闭和切换标签继续保留原有行为。
- **灵活窗口常驻触发补全（2026-07-31）**：首次单击文章或功能入口仍只创建可替换的预览标签；再次点击同一文章/功能入口会固定该标签。预览窗口内点击按钮、链接或表单控件，输入内容、提交表单、调整设置或拖动面板后，也会立即固定当前标签，避免用户已经操作过的内容被下一次普通打开替换。
- **摘要改为阅读区底部栏（2026-07-31）**：移除可拖拽悬浮窗，摘要、标签（含自动 AI 建议）和笔记共用三 tab `StickyBottomPanel`；工具栏按钮和底部“摘要”tab 均可触发生成或打开缓存，保留统一拉伸、切换、收起交互，并可与逐段翻译视图同时显示。阅读工具栏使用稳定标签“摘要 / 翻译 / 标签 / 笔记 / 专题”，不再切换为“显示/隐藏/关闭”文案；激活态以强调色浅底、边框和 `aria-pressed` 表达。重复的“询问 AI”工具栏按钮已移除，文章 AI 对话统一由右上角 AI 按钮打开，划词询问仍会自动展开对话栏。底栏普通按钮、标签卡片和输入区使用面板局部中性色，纸质主题下自动转为暖灰；主操作继续使用强调色。
- **摘要 Markdown 与文章标签排版（2026-07-31）**：摘要安全渲染器补齐 1–6 级标题、有序/无序嵌套列表、引用、分隔线和 fenced code block，并为底栏补齐对应排版样式；中栏标签位于标题下方、来源/时间上方，灵活窗口标签位于文章标题下方，两处均在容器内自然换行，不再挤压标题文本宽度。
- **翻译视图与阅读回顶入口（2026-07-31）**：删除译文正文顶部重复的“隐藏翻译，返回原文”悬浮按钮，翻译显示/隐藏统一由文章工具栏“翻译”按钮控制；阅读版滚动超过 240px 后，固定顶栏显示无文字的回到文章开头图标，支持平滑滚动与减少动态效果偏好。
- **本地日志用户界面已移除（2026-07-31）**：设置工作区不再提供日志查看或导出入口，`LogsPage` 前端代码已删除；Main 进程仍以 2 MB 轮转 JSONL 持久化脱敏后的启动、同步和 OPML 诊断事件，供开发排查使用。
- **代码审查修复（`8ca0998`，已发布）**：移除受 50 条分页限制却被误当作全集的 `allArticles` 缓存；删除确认和单源未读数改用数据库精确计数。切换订阅源会退出旧文章标签态，关闭文章或删除来源会同步清理文章快照；已读/星标写入改为成功后更新界面并显式报告失败。该版本曾将本地日志查看/导出接入设置工作区，相关用户界面现已移除。
...
- **Phase 4.3 VS Code 风格新手教程完成**：
  - 首次读取到 `onboardingCompleted=false` 时自动进入 12 步聚光引导；完成或跳过后持久化为 `true`，重启不再自动弹出。
  - 设置工作区新增“新手教程”入口，点击时先重置完成状态，再从第 1 步重新开始；结束后恢复进入教程前的灵活窗口标签与目录展开状态，不创建或固定额外标签。
  - 引导依次覆盖订阅源、添加入口、文章目录、同步、阅读区、阅读键目录切换、标签、笔记、文摘、专题、AI 和搜索；全屏遮罩封锁底层交互，卡片提供进度、上一步、下一步、跳过和开始使用。
  - `useTargetRect` 通过 `getBoundingClientRect()`、`ResizeObserver`、DOM 变化、窗口缩放、滚动和全屏事件持续刷新聚光位置；目标暂时缺失时限时等待后自动前进。
  - **动画性能优化（2026-07-31）**：目标测量按动画帧合并并跳过相同坐标，忽略教程自身 DOM 变化，减少无效 React 渲染；步骤切换不再重建整张卡片，聚光框改用合成层位移，仅在切换步骤时启用 220ms 过渡，窗口缩放和滚动时即时跟随、不再追逐旧坐标。
  - 中英文文案、深浅主题与减少动态效果均已适配。`OnboardingOverlay.test.ts` 覆盖步骤、完成/跳过和定位边界；`smoke:onboarding` 使用真实临时数据库覆盖首次启动、12 个目标、窗口缩放、完成/跳过持久化、设置重开与二次启动不再弹出。
...
- **25 个 smoke 命令 + 160 个自动化测试通过**（另 6 个需外网的真实 Feed 测试按设计跳过）。Phase 4.3 新增 `smoke:onboarding`，使用真实临时数据库分别启动两次，验证首次自动显示、完整 12 步、设置重开、完成/跳过持久化和重启不再自动显示。
...
2. Phase 5：v1.0.0 已完成 GitHub Actions 双平台发布，Windows 真机与 Linux 验证 / 课程交付资料准备（进行中）

详细任务和验收标准位于 `PLAN.md`，本文件不重复记录任务级进度。

## 近期记录（按 commit 倒序）

- **macOS 单层标题栏（本次修改）**：修复默认原生标题栏与 React 工具栏叠成两行的问题；macOS 改用 `hiddenInset`，以自定义交通灯坐标对齐标题和搜索框，并在不暴露 Node 能力的前提下由 preload 标记平台，顶栏补齐交通灯安全区、窗口拖动区和交互控件 no-drag 区。新增窗口契约测试并完成真实窗口视觉与交互复核。
...
- **笔记页长标题越界修复（本次修改）**：原生文章下拉框受最长 option 的 min-content 宽度影响，会撑出灵活窗口并触发外层横向滚动；为笔记页、选择器和表单补齐 `min-width: 0`，下拉框改用 `flex: 1 1 0` 在剩余空间内收缩并省略长标题，集成 smoke 使用超长标题断言页面、下拉框和滚动宽度均不越界。
- **新手教程动画性能优化（本次修改）**：修复全局 MutationObserver 重复测量、相同坐标重复渲染、卡片逐步卸载重建和聚光框连续过渡追赶导致的卡顿；新增卡片节点复用、步骤过渡状态和合成层位移的单测与 Electron smoke 断言。
- **本地日志界面移除（本次提交）**：删除设置工作区中的“本地日志”入口与 `LogsPage` 前端组件、样式和测试；保留 Main 进程脱敏日志，并在集成 smoke 中增加“入口和页面均不存在”的回归断言。
- **Phase 4.3 初版（历史记录）**：完成 VS Code 风格 8 步新手教程、首次启动自动触发、设置页重新查看、完成/跳过持久化、动态聚光定位、中英文文案、组件测试与真实数据库双启动 Electron smoke；v0.3.4 已扩展为 12 步。
