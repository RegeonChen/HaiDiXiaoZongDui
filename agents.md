...
- **AI 延迟与全链路中文错误提示加固（2026-08-02，待下个版本交付）**：逐段翻译从完全串行改为受控双并发，并按段落长度动态限制输出；摘要、标签和文章问答去除图片 URL，对超长正文保留开头/中段/结尾代表性内容，任务超时收紧为 45–60 秒。Main 进程统一分类 AI 鉴权、限流、模型、网络、代理、证书、空响应和格式错误；Renderer 将同步、Feed、OPML、磁盘、数据库及 AI 错误转换为带处理建议的中文文案，批量操作展示逐项失败原因。双端 typecheck、203 项单测、生产构建、摘要/翻译/标签/AI 对话、Feed actions、OPML 导出和 Phase 3 集成 Electron smoke 均通过；真实外部 Provider 摘要/翻译/标签仍受“无 API Key”环境限制而跳过。

- **功能工作区全屏布局统一（2026-08-02，待下个版本交付）**：标签、笔记、专题、OPML 导出、通用设置、AI 设置和专题详情统一使用 1040px 居中内容轴；展开目录或窄窗口时按可用宽度自然收缩，收起目录或全屏时不再出现部分页面铺满、部分页面居中的差异。专题详情同步统一为页头、标签栏与内容卡片结构。独立的 `workspace-pages.css` 在页面组件样式之后加载，避免历史页面宽度再次覆盖统一规则；`smoke:phase4.2` 在两级目录均收起时测得标签、笔记、专题宽度均为 1040px、左右留白均为 97px，OPML、专题和完整工作台 smoke 均通过。
...
2. Phase 5：v0.3.5 已完成 GitHub Actions 双平台发布，Windows 真机与 Linux 验证 / 课程交付资料准备（进行中）
